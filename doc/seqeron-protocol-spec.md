...
**5** = seqeron's own end-to-end probe (`sbe-probe.xml` 214, one `ProbeMarker`), **6** = the `seqeron-examples`
ping, which both of them submit (no schema at all: eight raw bytes, which §13.2 admits). **4** is the only one that crosses application boundaries and so the only one §6.4's
