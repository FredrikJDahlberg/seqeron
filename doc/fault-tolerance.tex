...
fatal handler wired by \texttt{SequencerNode}, exiting the process with
...
without taking a snapshot, and \texttt{SequencerNode} wires
...
\item
  ...
  logic let an unrelated \texttt{OrderExecClient} restart promote a
...
\texttt{OrderExecClient}, \texttt{BasicDataClient},
...
(\texttt{src/main/java/org/limitless/phixeron/replayer/ReplayerService.java},
...
\texttt{ClusterStreamReceiver}/\texttt{ClusterStreamClient} is the
...
\texttt{PortfolioQueryRequest} handling (\texttt{OrderExecClient}) needs
...
\texttt{BasicDataClient}/\texttt{Gateways} treat reference data with the
...
\item
  ...
  \texttt{OrderExecClient} beside a running cluster rather than
