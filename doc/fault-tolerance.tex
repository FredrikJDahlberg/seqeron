...
\makeatother
\hypersetup{
  pdftitle={Fault tolerance and recovery --- seqeron},
  colorlinks=true,
  linkcolor={blue},
  filecolor={Maroon},
  citecolor={Blue},
...
\title{Fault tolerance and recovery --- seqeron}
\author{}
\date{}
...
seqeron}\label{fault-tolerance-and-recovery-seqeron}
...
(\texttt{src/main/java/org/limitless/seqeron/sequencer/SequencerService.java:295-300,544-550});
\texttt{clusterctl\ shutdown} uses Aeron's \texttt{ABORT} action, which
takes no snapshot, never \texttt{SHUTDOWN}
(\texttt{src/main/java/org/limitless/seqeron/tools/ClusterCtl.java:151-185}
...
Raft election is Aeron Cluster's own mechanism; seqeron's contribution
...
(\texttt{src/main/cpp/org/limitless/seqeron/sequencer/ClusterStreamSender.hpp:632-689}).
...
entirely (\texttt{FATAL\_RECORDING\_GONE}). The same 1 Hz heartbeat that
...
reasoning bounds \texttt{scheduleHeartbeat}
...
No \texttt{ClusterHeartbeat} from the co-located tap for
\texttt{TAP\_STALL\_TIMEOUT\_MS} (20s = 20× the 1 Hz heartbeat period) &
...
\texttt{ClusterHeartbeat} frames being watched for, so it would freeze along with a
...
(\texttt{src/main/java/org/limitless/seqeron/replayer/server/ReplayerService.java},
...
\item
  ...
  \texttt{seqeron\_sequencer\_tap\_stalled} (latches at 1 when a node
  is about to terminate itself, §1.3),
  \texttt{seqeron\_sequencer\_gateway\_promotion\_total} (§2.2), the
  \texttt{seqeron\_replayer\_*} family (§3.1), and
  \texttt{seqeron\_node\_up} (an aggregator-synthesized per-node
...
\item
  \textbf{\texttt{cluster/src/test/scripts/chaos-runner.sh}} --- randomized
...
\item
  \textbf{\texttt{cluster/src/test/scripts/replay-bench.sh}} --- times a cold
