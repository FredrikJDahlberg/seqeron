...
#ifndef _ORG_LIMITLESS_SEQERON_CLUSTER_SBE_VARASCIIENCODING_CXX_H_
#define _ORG_LIMITLESS_SEQERON_CLUSTER_SBE_VARASCIIENCODING_CXX_H_
...
public:
    ...
    VarAsciiEncoding(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        VarAsciiEncoding(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    VarAsciiEncoding(
        char *buffer,
        const std::uint64_t bufferLength) :
        VarAsciiEncoding(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }

    VarAsciiEncoding &wrap(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    ...
    VarAsciiEncoding &length(const std::uint32_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, VarAsciiEncoding &writer)
{
    builder << '{';
    builder << R"("length": )";
