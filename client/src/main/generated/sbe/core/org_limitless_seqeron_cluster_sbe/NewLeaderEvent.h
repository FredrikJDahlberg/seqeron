...
#ifndef _ORG_LIMITLESS_SEQERON_CLUSTER_SBE_NEWLEADEREVENT_CXX_H_
#define _ORG_LIMITLESS_SEQERON_CLUSTER_SBE_NEWLEADEREVENT_CXX_H_
...
public:
    ...
    NewLeaderEvent(char *buffer, const std::uint64_t bufferLength) :
        NewLeaderEvent(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    NewLeaderEvent(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        NewLeaderEvent(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    NewLeaderEvent &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    NewLeaderEvent &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        NewLeaderEvent skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    NewLeaderEvent &leadershipTermId(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    NewLeaderEvent &clusterSessionId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    NewLeaderEvent &leaderMemberId(const std::int32_t value) SBE_NOEXCEPT
    {
    ...
    SBE_NODISCARD static const char *ingressEndpointsMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static const char *ingressEndpointsCharacterEncoding() SBE_NOEXCEPT
    {
        return "US-ASCII";
    }

    static SBE_CONSTEXPR std::uint64_t ingressEndpointsSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    bool ingressEndpointsInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    static SBE_CONSTEXPR std::uint16_t ingressEndpointsId() SBE_NOEXCEPT
    {
        return 4;
    }

    static SBE_CONSTEXPR std::uint64_t ingressEndpointsHeaderLength() SBE_NOEXCEPT
    {
        return 4;
    }

    SBE_NODISCARD std::uint32_t ingressEndpointsLength() const
    {
        std::uint32_t length;
    ...
    std::uint64_t skipIngressEndpoints()
    {
    ...
    SBE_NODISCARD const char *ingressEndpoints()
    {
        std::uint32_t lengthFieldValue;
    ...
    std::uint64_t getIngressEndpoints(char *dst, const std::uint64_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    NewLeaderEvent &putIngressEndpoints(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    std::string getIngressEndpointsAsString()
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
        sbePosition(lengthPosition + lengthOfLengthField);
    ...
    std::string getIngressEndpointsAsJsonEscapedString()
    {
        std::ostringstream oss;
        std::string s = getIngressEndpointsAsString();
    ...
    std::string_view getIngressEndpointsAsStringView()
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
        sbePosition(lengthPosition + lengthOfLengthField);
    ...
    NewLeaderEvent &putIngressEndpoints(const std::string &str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
        }
        return putIngressEndpoints(str.data(), static_cast<std::uint32_t>(str.length()));
    }

    #ifdef SBE_USE_STRING_VIEW
    NewLeaderEvent &putIngressEndpoints(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const NewLeaderEvent &_writer)
{
    NewLeaderEvent writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << ", ";
    builder << R"("ingressEndpoints": )";
    builder << '"' <<
        writer.getIngressEndpointsAsJsonEscapedString().c_str() << '"';
...
SBE_NODISCARD static std::size_t computeLength(std::size_t ingressEndpointsLength = 0)
{
...
#pragma GCC diagnostic ignored "-Wtype-limits"
    ...
    length += ingressEndpointsHeaderLength();
    if (ingressEndpointsLength > 1073741824LL)
    {
        throw std::runtime_error("ingressEndpointsLength too long for length type [E109]");
    }
    length += ingressEndpointsLength;
