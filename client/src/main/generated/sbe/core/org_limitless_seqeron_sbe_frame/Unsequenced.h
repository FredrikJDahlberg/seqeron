...
#ifndef _ORG_LIMITLESS_SEQERON_SBE_FRAME_UNSEQUENCED_CXX_H_
#define _ORG_LIMITLESS_SEQERON_SBE_FRAME_UNSEQUENCED_CXX_H_
...
public:
    ...
    Unsequenced(char *buffer, const std::uint64_t bufferLength) :
        Unsequenced(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    Unsequenced(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        Unsequenced(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    Unsequenced &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    Unsequenced &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        Unsequenced skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
private:
    UnsequencedHeader m_header;

public:
    SBE_NODISCARD UnsequencedHeader &header()
    {
    ...
    Unsequenced &putPayload(const char *src, const std::uint16_t length)
    {
        std::uint64_t lengthOfLengthField = 2;
        std::uint64_t lengthPosition = sbePosition();
    ...
    Unsequenced &putPayload(const std::string &str)
    {
        if (str.length() > 65534)
        {
    ...
    #ifdef SBE_USE_STRING_VIEW
    Unsequenced &putPayload(const std::string_view str)
    {
        if (str.length() > 65534)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const Unsequenced &_writer)
{
    Unsequenced writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << R"("payload": )";
    builder << '"' <<
        writer.skipPayload() << " bytes of raw data\"";
    builder << '}';
...
SBE_NODISCARD static std::size_t computeLength(std::size_t payloadLength = 0)
{
