...
#ifndef _ORG_LIMITLESS_SEQERON_SBE_FRAME_SEQUENCED_CXX_H_
#define _ORG_LIMITLESS_SEQERON_SBE_FRAME_SEQUENCED_CXX_H_
...
public:
    ...
    Sequenced(char *buffer, const std::uint64_t bufferLength) :
        Sequenced(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    Sequenced(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        Sequenced(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    Sequenced &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    Sequenced &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        Sequenced skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
private:
    SequencedHeader m_header;

public:
    SBE_NODISCARD SequencedHeader &header()
    {
    ...
    Sequenced &putPayload(const char *src, const std::uint16_t length)
    {
        std::uint64_t lengthOfLengthField = 2;
        std::uint64_t lengthPosition = sbePosition();
    ...
    Sequenced &putPayload(const std::string &str)
    {
        if (str.length() > 65534)
        {
    ...
    #ifdef SBE_USE_STRING_VIEW
    Sequenced &putPayload(const std::string_view str)
    {
        if (str.length() > 65534)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const Sequenced &_writer)
{
    Sequenced writer(
        _writer.m_buffer,
        _writer.m_offset,
