...
#ifndef _ORG_LIMITLESS_SEQERON_SBE_FRAME_CLUSTERSTOPPED_CXX_H_
#define _ORG_LIMITLESS_SEQERON_SBE_FRAME_CLUSTERSTOPPED_CXX_H_
...
public:
    ...
    ClusterStopped(char *buffer, const std::uint64_t bufferLength) :
        ClusterStopped(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    ClusterStopped(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        ClusterStopped(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    ClusterStopped &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    ClusterStopped &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        ClusterStopped skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    ClusterStopped &correlationId(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const ClusterStopped &_writer)
{
    ClusterStopped writer(
        _writer.m_buffer,
        _writer.m_offset,
