...
class IngressStallPolicyTest {
    private static final long STALL_MS = TimeUnit.SECONDS.toMillis(10);
    private static final long ALERT_MS = TimeUnit.SECONDS.toMillis(1);
    /** Arbitrary non-zero clock origin — 0 is the policy's "no block in progress" sentinel. */
    private static final long T0 = TimeUnit.HOURS.toMillis(5);

    private static long seconds(final long s) {
        return T0 + TimeUnit.SECONDS.toMillis(s);
    }

    private final IngressStallPolicy policy = new IngressStallPolicy(STALL_MS, ALERT_MS);
    ...
    void closedIsRetriedForTheLengthOfAnElection() {
        ...
        policy.onOffered();
        assertEquals(0, policy.blockedMs(seconds(5)));
    }
    ...
    void backPressureRetriesQuietlyThenAlerts() {
        ...
        assertEquals(Action.RETRY, policy.onOfferFailed(T0 + ALERT_MS / 2, Publication.BACK_PRESSURED, false, false));
        assertEquals(Action.ALERT, policy.onOfferFailed(T0 + ALERT_MS, Publication.BACK_PRESSURED, false, false));
        assertEquals(Action.RETRY,
                     policy.onOfferFailed(T0 + ALERT_MS + 1, Publication.BACK_PRESSURED, false, false));
    }
    ...
    void sustainedBlockStalls() {
        ...
        assertEquals(Action.STALLED,
                     policy.onOfferFailed(T0 + STALL_MS, Publication.BACK_PRESSURED, false, false));
    }
    ...
    void aLandedOfferResetsTheClock() {
        ...
        assertEquals(Action.ALERT,
                     policy.onOfferFailed(T0 + STALL_MS - 1, Publication.BACK_PRESSURED, false, false));
        policy.onOffered();
    ...
    void firstFailureAnchorsRatherThanJudges() {
        ...
        assertEquals(0, policy.blockedMs(seconds(100)));
        assertEquals(TimeUnit.SECONDS.toMillis(1), policy.blockedMs(seconds(101)));
    }
}
