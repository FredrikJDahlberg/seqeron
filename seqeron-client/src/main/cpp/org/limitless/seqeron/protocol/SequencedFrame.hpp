...
inline FrameView unwrapFrame(const char* const frame, const std::uint64_t length)
{
    ...
    if (frameTemplateId == sbe::frame::Sequenced::sbeTemplateId())
    {
        ...
        // SBE's own bounds check from raising it as a throw out of the poll thread, where Image::poll
        // hands it to the error handler and advances past the frame anyway.
        if (sbe::frame::Sequenced::sbeBlockAndHeaderLength() + sbe::frame::Sequenced::payloadHeaderLength() +
                payloadLength >
            length)
        {
            return view;
