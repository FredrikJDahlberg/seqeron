...
#ifndef _ORG_LIMITLESS_SEQERON_SBE_FRAME_CLUSTERSTARTED_CXX_H_
#define _ORG_LIMITLESS_SEQERON_SBE_FRAME_CLUSTERSTARTED_CXX_H_
...
public:
    ...
    ClusterStarted(char *buffer, const std::uint64_t bufferLength) :
        ClusterStarted(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    ClusterStarted(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        ClusterStarted(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    ClusterStarted &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    ClusterStarted &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        ClusterStarted skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    ClusterStarted &correlationId(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const ClusterStarted &_writer)
{
    ClusterStarted writer(
        _writer.m_buffer,
        _writer.m_offset,
