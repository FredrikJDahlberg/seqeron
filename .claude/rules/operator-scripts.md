---
paths:
  - "seqeron-service/src/main/scripts/**"
---

# Operator scripts

`seqeron-service/src/main/scripts` holds the operator and cluster-lifecycle scripts; `ports.sh` (the port formula, mirrored
...
the scripts sit at `seqeron-service/src/main/scripts` — and `seqeron_require_jar` then finds the uber jar in `lib/` or
