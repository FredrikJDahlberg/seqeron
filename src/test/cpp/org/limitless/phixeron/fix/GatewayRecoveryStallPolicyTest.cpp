...
TEST(GatewayRecoveryStallPolicy, ColdStartNeverTripsRegardlessOfHowLongItTakes)
{
    ...
    {
        EXPECT_FALSE(policy.onNotCaughtUp(seconds(s), STUCK)) << "second " << s
...
TEST(GatewayRecoveryStallPolicy, RecoveryThatKeepsDispatchingIsNeverFencedHoweverLongItRuns)
{
    ...
    {
        EXPECT_FALSE(policy.onNotCaughtUp(seconds(s), 1000 + s)) << "second " << s
            << ": a recovery advancing its globalSeqNo is converging, however long it takes";
    }
}
