...
TEST_P(PendingSendsProperty, LogsEveryFrameOnceInOrderAcrossFailovers)
{
    ...
    {
        const int action = model.below(100);
        if (action < 30)
        {
            model.send();
        }
        ...
        {
            model.commit(model.below(4));
        }
        else if (action < 55)
        {
            model.tap.push_back({ SIBLING, model.below(8) });
        }
        else if (action < 80)
        {
            model.deliver(model.below(4));
        }
        else if (action < 85)
        {
        ...
        else if (action < 87)
        {
            ...
            model.clusterTerm += 1 + model.below(2);
            model.tap.push_back({ LEADERSHIP_CHANGED, model.clusterTerm });
            model.oldLeaderTakesIngress = model.below(2) == 0;
        }
