...
class MetricsExporterTest {
    ...
    void appCountersOfOneTypeShareOneHeader() {
        addAppCounter(PhixeronCounters.APP_RECOVERY_STALLED_TYPE_ID, MEMBER, 9, 0);
        addAppCounter(PhixeronCounters.APP_RECOVERY_STALLED_TYPE_ID, MEMBER, 10, 1);

        final String body = new MetricsExporter(counters).renderMetrics();

        assertEquals(1, count(body, "# HELP phixeron_app_recovery_stalled "),
                     "a repeated HELP line makes Prometheus reject the entire scrape");
        assertEquals(1, count(body, "# TYPE phixeron_app_recovery_stalled "));
        assertTrue(body.contains("phixeron_app_recovery_stalled{member=\"1\",client=\"9\"} 0"), body);
        assertTrue(body.contains("phixeron_app_recovery_stalled{member=\"1\",client=\"10\"} 1"), body);
    }
    ...
    void distinctMetricsKeepDistinctHeaders() {
        addAppCounter(PhixeronCounters.APP_RECOVERY_STALLED_TYPE_ID, MEMBER, 9, 0);
        addCounter(PhixeronCounters.SEQUENCER_GLOBAL_SEQ_NO_TYPE_ID, MEMBER, 42);
        addAppCounter(PhixeronCounters.APP_RECOVERY_STALLED_TYPE_ID, MEMBER, 10, 1);

        final String body = new MetricsExporter(counters).renderMetrics();

        assertEquals(1, count(body, "# HELP phixeron_app_recovery_stalled "));
        assertEquals(1, count(body, "# HELP phixeron_sequencer_global_seq_no "));
        ...
        assertTrue(body.indexOf("client=\"10\"") < body.indexOf("# HELP phixeron_sequencer_global_seq_no ")
                       || body.indexOf("# HELP phixeron_sequencer_global_seq_no ") < body.indexOf("client=\"9\""),
                   body);
    }
    ...
    void nodeScopedCounterHasNoClientLabel() {
        ...
        assertTrue(new MetricsExporter(counters).renderMetrics()
                       .contains("phixeron_sequencer_global_seq_no{member=\"1\"} 42"));
    }

    @Test
    @DisplayName("a counter phixeron does not own is skipped")
    ...
    private void addCounter(final int typeId, final int memberId, final long value) {
        counters.newCounter("phixeron member=" + memberId, typeId,
                            keyBuffer -> keyBuffer.putInt(PhixeronCounters.KEY_MEMBER_ID_OFFSET, memberId))
            .set(value);
    }

    private void addAppCounter(final int typeId, final int memberId, final int clientId, final long value) {
        counters
            .newCounter("phixeron member=" + memberId + " client=" + clientId, typeId,
                        keyBuffer -> {
                            keyBuffer.putInt(PhixeronCounters.KEY_MEMBER_ID_OFFSET, memberId);
                            keyBuffer.putInt(PhixeronCounters.KEY_CLIENT_ID_OFFSET, clientId);
                        })
            .set(value);
    }
