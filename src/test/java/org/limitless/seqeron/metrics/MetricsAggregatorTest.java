 ...
 * healthy node's metrics and the {@code seqeron_node_up} gauge with it — so a single bad node blinded
...
class MetricsAggregatorTest {
    ...
    void labelledSamplesGroupByName() {
        MetricsAggregator.parseResponse("seqeron_app_recovery_stalled{member=\"0\",client=\"9\"} 0\n"
                                            + "seqeron_app_recovery_stalled{member=\"0\",client=\"10\"} 1\n",
                                        help, type, samples);

        assertEquals(1, samples.size());
        assertEquals("seqeron_app_recovery_stalled{member=\"0\",client=\"9\"} 0\n"
                         + "seqeron_app_recovery_stalled{member=\"0\",client=\"10\"} 1\n",
                     samples.get("seqeron_app_recovery_stalled").toString());
    }

    @Test
    @DisplayName("a targets entry with no separator stops start-up and names itself")
    void targetWithoutSeparatorIsRejected() {
        // Skipping it would drop the node from the aggregate entirely — not even seqeron_node_up 0 —
