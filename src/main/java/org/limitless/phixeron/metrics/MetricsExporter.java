...
public final class MetricsExporter {
    ...
    private static Map<Integer, MetricMeta> buildMetricsByTypeId() {
        final List<Map.Entry<Integer, MetricMeta>> entries = List.of(
            Map.entry(PhixeronCounters.SEQUENCER_GLOBAL_SEQ_NO_TYPE_ID, new MetricMeta(
                "phixeron_sequencer_global_seq_no", "Last globalSeqNo emitted on this node's tap.", "gauge")),
            Map.entry(PhixeronCounters.SEQUENCER_TAP_BACKPRESSURE_ALERTS_TYPE_ID, new MetricMeta(
                "phixeron_sequencer_tap_backpressure_alerts_total",
                "Count of times the tap-emit back-pressure alert threshold has fired.", "counter")),
            Map.entry(PhixeronCounters.SEQUENCER_REJECTED_INGRESS_COUNT_TYPE_ID, new MetricMeta(
                "phixeron_sequencer_rejected_ingress_total",
                "Count of malformed ingress messages skipped by Sequencer.sequenceMessage.", "counter")),
            Map.entry(PhixeronCounters.SEQUENCER_LEADERSHIP_CHANGE_COUNT_TYPE_ID, new MetricMeta(
                "phixeron_sequencer_leadership_change_total",
                "Count of leadership changes this node has observed and sequenced.", "counter")),
            Map.entry(PhixeronCounters.SEQUENCER_CURRENT_LEADER_MEMBER_ID_TYPE_ID, new MetricMeta(
                "phixeron_sequencer_current_leader_member_id",
                "memberId of the leader last recorded by this node's Sequencer.", "gauge")),
            Map.entry(PhixeronCounters.SEQUENCER_LAST_TICK_TIMESTAMP_TYPE_ID, new MetricMeta(
                "phixeron_sequencer_last_tick_timestamp_ms",
                "Consensus timestamp of the last 1Hz Tick emitted.", "gauge")),
            Map.entry(PhixeronCounters.SEQUENCER_GATEWAY_PROMOTION_COUNT_TYPE_ID, new MetricMeta(
                "phixeron_sequencer_gateway_promotion_total",
                "Count of standby-promotion GatewayActive frames emitted on a gateway session close.", "counter")),
            ...
            Map.entry(PhixeronCounters.REPLAYER_READY_TYPE_ID, new MetricMeta(
                "phixeron_replayer_ready",
                "1 once the co-located tap recording is visible and replay requests are being served.", "gauge")),
            Map.entry(PhixeronCounters.REPLAYER_ACTIVE_SLOTS_TYPE_ID, new MetricMeta(
                "phixeron_replayer_active_slots", "Current count of in-flight replays.", "gauge")),
            Map.entry(PhixeronCounters.REPLAYER_PENDING_REQUESTS_TYPE_ID, new MetricMeta(
                "phixeron_replayer_pending_requests",
                "Current count of replay requests waiting for a free slot.", "gauge")),
            Map.entry(PhixeronCounters.REPLAYER_REPLAYS_SERVED_COUNT_TYPE_ID, new MetricMeta(
    ...
    public static void main(final String[] args) throws IOException {
        ...
        System.out.printf("[MetricsExporter/%d] serving /metrics on port %d from aeronDir=%s%n",
                           MEMBER_ID, PORT, AERON_DIR);
    ...
    private void handleMetrics(final HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            final byte[] error = ("aeron counters unavailable: " + ex.getMessage() + "\n").getBytes(StandardCharsets.UTF_8);
