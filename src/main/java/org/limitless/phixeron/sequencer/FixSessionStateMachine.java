...
public final class FixSessionStateMachine {
    ...
    static final int BLOCK_SENDER_OFFSET = BLOCK_START;
    ...
    static final int FIX_SESSION_CONNECT = 0x01;
    static final int FIX_SESSION_DISCONNECT = 0x02;
    static final int FIX_HEARTBEAT = TEMPLATE_HEARTBEAT;
    static final int FIX_TEST_REQUEST = TEMPLATE_TEST_REQUEST;
    static final int FIX_RESEND_REQUEST = TEMPLATE_RESEND_REQUEST;
    static final int FIX_SEQUENCE_RESET = TEMPLATE_SEQUENCE_RESET;
    static final int FIX_LOGOUT = TEMPLATE_LOGOUT;
    static final int FIX_LOGON = TEMPLATE_LOGON;
    ...
    // ── Runtime-only state (not checkpointed) ──────────────────────────────────
    ...
    public long heartbeatTimerDeadline() {
        return heartbeatTimerDeadline;
    }

    public long testReqTimer() {
        return testReqTimer;
    }

    public long testReqTimerDeadline() {
        return testReqTimerDeadline;
    }

    public String pendingTestReqId() {
        return pendingTestReqId;
    }
    ...
    public Command[] onEvent(
        ...
        final Command[] result = switch (discriminator) {
            case FIX_SESSION_CONNECT -> onSessionConnect(buffer, offset, nowMs);
            case FIX_SESSION_DISCONNECT -> onSessionDisconnect();
            case FIX_LOGON -> onInboundLogon(buffer, offset, nowMs);
            case FIX_LOGOUT -> onInboundLogout(buffer, offset, nowMs);
            case FIX_HEARTBEAT -> onInboundHeartbeat(buffer, offset, nowMs);
            case FIX_TEST_REQUEST -> onInboundTestRequest(buffer, offset);
            case FIX_RESEND_REQUEST -> onInboundResendRequest(buffer, offset);
            case FIX_SEQUENCE_RESET -> onInboundSequenceReset(buffer, offset);
        ...
        if (postFailoverPendingTestReqReset
            && discriminator != FIX_SESSION_CONNECT
            && discriminator != FIX_SESSION_DISCONNECT) {
    ...
    public Command[] onTimer(final long correlationId, final long nowMs) {
        if (correlationId == heartbeatTimer && heartbeatTimer != 0) {
            return onHeartbeatTimerExpiry(nowMs);
        }
        if (correlationId == testReqTimer && testReqTimer != 0) {
            return onTestReqTimerExpiry(nowMs);
        }
        return Command.NONE;
    }
    ...
    private Command[] onSessionConnect(final DirectBuffer buffer, final int offset, final long nowMs) {
        if (sessionPhase != SessionPhase.DISCONNECTED) {
            return Command.NONE;
        }
    ...
    private Command[] onInboundLogon(final DirectBuffer buffer, final int offset, final long nowMs) {
        if (sessionPhase != SessionPhase.LOGON_PENDING) {
            return Command.NONE;
        }
    ...
    private Command[] onInboundHeartbeat(final DirectBuffer buffer, final int offset, final long nowMs) {
    ...
    private Command[] onInboundLogout(final DirectBuffer buffer, final int offset, final long nowMs) {
        if (sessionPhase == SessionPhase.DISCONNECTED) {
            return Command.NONE;
        }
    ...
    private Command[] onInboundApplicationMessage(final int fixMessageId,
                                                  final DirectBuffer buffer,
                                                  final int offset,
                                                  final int length) {
        if (sessionPhase != SessionPhase.ACTIVE) {
     ...
     */
    public Command[] onResendFragment(final DirectBuffer buffer, final int offset, final int length, final long nowMs) {
    ...
    public Command[] flushMemoryStorageTail(final long fromSeqNum, final long endSeqNo) {
        if (!memoryStorage.covers(fromSeqNum, endSeqNo)) {
            return Command.NONE;
        }
    ...
    private SendCommand buildResendRequest(final long beginSeqNo, final long endSeqNo, final long nowMs) {
        ...
        pos = writeStandardBlock(outboundBuf, pos, outboundSeqNum, nowMs);
        outboundBuf.putInt(pos, (int) (beginSeqNo & 0xFFFFFFFFL));
        pos += Integer.BYTES;
        outboundBuf.putInt(pos, (int) (endSeqNo & 0xFFFFFFFFL));
        pos += Integer.BYTES;
    ...
    private int writeTestReqId(
        ...
        for (int i = idLen; i < TEST_REQ_ID_LEN; i++) {
            buf.putByte(offset + i, (byte) 0);
        }
    ...
    private static String extractTestReqId(final DirectBuffer buffer, final int offset) {
        ...
        if ((buffer.getByte(start) & 0xFF) == 0) {
            return null;   // absent
        }
    ...
    /**
     * Serialises all checkpointed state into buf starting at offset.
    ...
    public int encodeTo(final MutableDirectBuffer buf, int offset) {
        ...
        buf.putLong(offset, sessionId);
        offset += Long.BYTES;
        buf.putByte(offset, sessionRole.encode());
        offset += Byte.BYTES;
        buf.putByte(offset, sessionPhase.encode());
        offset += Byte.BYTES;
        buf.putLong(offset, inboundSeqNum);
        offset += Long.BYTES;
        buf.putLong(offset, outboundSeqNum);
        offset += Long.BYTES;

        final byte[] pendingBytes = (pendingTestReqId != null)
            ? pendingTestReqId.getBytes()
            : new byte[0];
        buf.putShort(offset, (short) pendingBytes.length);
        offset += Short.BYTES;
        if (pendingBytes.length > 0) {
            buf.putBytes(offset, pendingBytes);
            offset += pendingBytes.length;
        }

        buf.putLong(offset, heartbeatTimer);
        offset += Long.BYTES;
        buf.putLong(offset, heartbeatTimerDeadline);
        offset += Long.BYTES;
        buf.putLong(offset, testReqTimer);
        offset += Long.BYTES;
        buf.putLong(offset, testReqTimerDeadline);
        offset += Long.BYTES;
        buf.putLong(offset, nextTimerId);
        offset += Long.BYTES;
        buf.putLong(offset, heartbeatIntervalMs);
        offset += Long.BYTES;

        buf.putBytes(offset, localCompId);
        offset += COMP_ID_LEN;
        buf.putBytes(offset, peerCompId);
        offset += COMP_ID_LEN;

        offset += memoryStorage.encodeTo(buf, offset);

        // outboundArchiveIndex
        buf.putInt(offset, outboundArchiveIndex.size());
        offset += Integer.BYTES;
        for (final var entry : outboundArchiveIndex.entrySet()) {
            buf.putLong(offset, entry.getKey());
            offset += Long.BYTES;
            buf.putLong(offset, entry.getValue().recordingId());
            offset += Long.BYTES;
            buf.putLong(offset, entry.getValue().startPosition());
            offset += Long.BYTES;
            buf.putLong(offset, entry.getValue().endPosition());
            offset += Long.BYTES;
        }
        return offset - start;
    }
    ...
    public static FixSessionStateMachine decodeFrom(final DirectBuffer buf, int offset) {
        final long sessionId = buf.getLong(offset);
        offset += Long.BYTES;
        final SessionRole role = SessionRole.decode(buf.getByte(offset));
        offset += Byte.BYTES;
        final SessionPhase ph = SessionPhase.decode(buf.getByte(offset));
        offset += Byte.BYTES;

        final FixSessionStateMachine fsm = new FixSessionStateMachine(sessionId, role, 2500);
        fsm.sessionPhase = ph;

        fsm.inboundSeqNum = buf.getLong(offset);
        offset += Long.BYTES;
        fsm.outboundSeqNum = buf.getLong(offset);
        offset += Long.BYTES;

        final int pendingLen = buf.getShort(offset) & 0xFFFF;
        offset += Short.BYTES;
        if (pendingLen > 0) {
        ...
        fsm.heartbeatTimer = buf.getLong(offset);
        offset += Long.BYTES;
        fsm.heartbeatTimerDeadline = buf.getLong(offset);
        offset += Long.BYTES;
        fsm.testReqTimer = buf.getLong(offset);
        offset += Long.BYTES;
        fsm.testReqTimerDeadline = buf.getLong(offset);
        offset += Long.BYTES;
        fsm.nextTimerId = buf.getLong(offset);
        offset += Long.BYTES;
        fsm.heartbeatIntervalMs = buf.getLong(offset);
        offset += Long.BYTES;

        buf.getBytes(offset, fsm.localCompId);
        offset += COMP_ID_LEN;
        buf.getBytes(offset, fsm.peerCompId);
        offset += COMP_ID_LEN;

        offset += fsm.memoryStorage.decodeFrom(buf, offset);

        final int archiveCount = buf.getInt(offset);
        offset += Integer.BYTES;
        for (int i = 0; i < archiveCount; i++) {
            final long seqNum = buf.getLong(offset);
            offset += Long.BYTES;
            final long recordingId = buf.getLong(offset);
            offset += Long.BYTES;
            final long startPos = buf.getLong(offset);
            offset += Long.BYTES;
            final long endPos = buf.getLong(offset);
            offset += Long.BYTES;
