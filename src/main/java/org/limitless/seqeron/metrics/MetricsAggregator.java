...
public final class MetricsAggregator {
    ...
    private static final String NODE_UP_NAME = "seqeron_node_up";
     ...
     * node's sample lines for that metric — plus the synthesized {@code seqeron_node_up} gauge.
     */
    ...
    static void parseResponse(final String response,
        ...
        for (final String line : response.split("\n")) {
            ...
            if (line.charAt(0) == '#') {
                ...
                // with it: every healthy node's metrics, and the seqeron_node_up gauge that is the one
