...
#ifndef _ORG_LIMITLESS_SEQERON_CLUSTER_SBE_CLUSTERACTIONREQUEST_CXX_H_
#define _ORG_LIMITLESS_SEQERON_CLUSTER_SBE_CLUSTERACTIONREQUEST_CXX_H_
...
public:
    ...
    ClusterActionRequest(char *buffer, const std::uint64_t bufferLength) :
        ClusterActionRequest(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    ClusterActionRequest(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        ClusterActionRequest(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    ClusterActionRequest &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    ClusterActionRequest &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    ClusterActionRequest &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    ClusterActionRequest &sbeRewind()
    {
    ...
    {
        ClusterActionRequest skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    ClusterActionRequest &leadershipTermId(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    ClusterActionRequest &logPosition(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    ClusterActionRequest &timestamp(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    SBE_NODISCARD static const char *actionMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static SBE_CONSTEXPR std::uint16_t actionId() SBE_NOEXCEPT
    {
        return 4;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t actionSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool actionInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }
    ...
    SBE_NODISCARD std::int32_t actionRaw() const SBE_NOEXCEPT
    {
        std::int32_t val;
    ...
    SBE_NODISCARD ClusterAction::Value action() const
    {
        std::int32_t val;
        std::memcpy(&val, m_buffer + m_offset + 24, sizeof(std::int32_t));
        return ClusterAction::get(SBE_LITTLE_ENDIAN_ENCODE_32(val));
    }

    ClusterActionRequest &action(const ClusterAction::Value value) SBE_NOEXCEPT
    {
    ...
    ClusterActionRequest &flags(const std::int32_t value) SBE_NOEXCEPT
    {
        std::int32_t val = SBE_LITTLE_ENDIAN_ENCODE_32(value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const ClusterActionRequest &_writer)
{
    ClusterActionRequest writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "ClusterActionRequest", )";
