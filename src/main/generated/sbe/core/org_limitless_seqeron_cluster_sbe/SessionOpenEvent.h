...
#ifndef _ORG_LIMITLESS_SEQERON_CLUSTER_SBE_SESSIONOPENEVENT_CXX_H_
#define _ORG_LIMITLESS_SEQERON_CLUSTER_SBE_SESSIONOPENEVENT_CXX_H_
...
public:
    ...
    SessionOpenEvent(char *buffer, const std::uint64_t bufferLength) :
        SessionOpenEvent(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    SessionOpenEvent(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        SessionOpenEvent(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    SessionOpenEvent &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    SessionOpenEvent &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    SessionOpenEvent &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        SessionOpenEvent skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    SessionOpenEvent &leadershipTermId(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    SessionOpenEvent &correlationId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    SessionOpenEvent &clusterSessionId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    SessionOpenEvent &timestamp(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    SessionOpenEvent &responseStreamId(const std::int32_t value) SBE_NOEXCEPT
    {
    ...
    SessionOpenEvent &putResponseChannel(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    SessionOpenEvent &putResponseChannel(const std::string &str)
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    SessionOpenEvent &putResponseChannel(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    SBE_NODISCARD static const char *encodedPrincipalMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static const char *encodedPrincipalCharacterEncoding() SBE_NOEXCEPT
    {
        return "null";
    }

    static SBE_CONSTEXPR std::uint64_t encodedPrincipalSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    bool encodedPrincipalInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    static SBE_CONSTEXPR std::uint16_t encodedPrincipalId() SBE_NOEXCEPT
    {
        return 8;
    }

    static SBE_CONSTEXPR std::uint64_t encodedPrincipalHeaderLength() SBE_NOEXCEPT
    {
        return 4;
    }

    SBE_NODISCARD std::uint32_t encodedPrincipalLength() const
    {
        std::uint32_t length;
        std::memcpy(&length, m_buffer + sbePosition(), sizeof(std::uint32_t));
        return SBE_LITTLE_ENDIAN_ENCODE_32(length);
    ...
    std::uint64_t skipEncodedPrincipal()
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
        std::uint32_t lengthFieldValue;
    ...
    SBE_NODISCARD const char *encodedPrincipal()
    {
        std::uint32_t lengthFieldValue;
        std::memcpy(&lengthFieldValue, m_buffer + sbePosition(), sizeof(std::uint32_t));
        const char *fieldPtr = m_buffer + sbePosition() + 4;
    ...
    std::uint64_t getEncodedPrincipal(char *dst, const std::uint64_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
        sbePosition(lengthPosition + lengthOfLengthField);
    ...
    SessionOpenEvent &putEncodedPrincipal(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
        std::uint32_t lengthFieldValue = SBE_LITTLE_ENDIAN_ENCODE_32(length);
    ...
    std::string getEncodedPrincipalAsString()
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
        sbePosition(lengthPosition + lengthOfLengthField);
    ...
    std::string getEncodedPrincipalAsJsonEscapedString()
    {
        std::ostringstream oss;
        std::string s = getEncodedPrincipalAsString();
    ...
    #ifdef SBE_USE_STRING_VIEW
    std::string_view getEncodedPrincipalAsStringView()
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
        sbePosition(lengthPosition + lengthOfLengthField);
    ...
    SessionOpenEvent &putEncodedPrincipal(const std::string &str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
        }
        return putEncodedPrincipal(str.data(), static_cast<std::uint32_t>(str.length()));
    }

    #ifdef SBE_USE_STRING_VIEW
    SessionOpenEvent &putEncodedPrincipal(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const SessionOpenEvent &_writer)
{
    SessionOpenEvent writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "SessionOpenEvent", )";
    ...
    builder << ", ";
    builder << R"("encodedPrincipal": )";
    builder << '"' <<
        writer.skipEncodedPrincipal() << " bytes of raw data\"";
    builder << '}';
...
SBE_NODISCARD static std::size_t computeLength(
    ...
    std::size_t encodedPrincipalLength = 0)
{
...
#pragma GCC diagnostic ignored "-Wtype-limits"
    ...
    length += encodedPrincipalHeaderLength();
    if (encodedPrincipalLength > 1073741824LL)
    {
        throw std::runtime_error("encodedPrincipalLength too long for length type [E109]");
    }
    length += encodedPrincipalLength;
