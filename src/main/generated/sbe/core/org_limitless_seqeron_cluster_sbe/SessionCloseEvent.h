...
#ifndef _ORG_LIMITLESS_SEQERON_CLUSTER_SBE_SESSIONCLOSEEVENT_CXX_H_
#define _ORG_LIMITLESS_SEQERON_CLUSTER_SBE_SESSIONCLOSEEVENT_CXX_H_
...
public:
    ...
    SessionCloseEvent(char *buffer, const std::uint64_t bufferLength) :
        SessionCloseEvent(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    SessionCloseEvent(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        SessionCloseEvent(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    SessionCloseEvent &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    SessionCloseEvent &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    SessionCloseEvent &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        SessionCloseEvent skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    SessionCloseEvent &leadershipTermId(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    SessionCloseEvent &clusterSessionId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    SessionCloseEvent &timestamp(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    SBE_NODISCARD static const char *closeReasonMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static SBE_CONSTEXPR std::uint16_t closeReasonId() SBE_NOEXCEPT
    {
        return 4;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t closeReasonSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool closeReasonInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t closeReasonEncodingOffset() SBE_NOEXCEPT
    {
        return 24;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t closeReasonEncodingLength() SBE_NOEXCEPT
    {
        return 4;
    }

    SBE_NODISCARD std::int32_t closeReasonRaw() const SBE_NOEXCEPT
    {
        std::int32_t val;
    ...
    SBE_NODISCARD CloseReason::Value closeReason() const
    {
        std::int32_t val;
        std::memcpy(&val, m_buffer + m_offset + 24, sizeof(std::int32_t));
        return CloseReason::get(SBE_LITTLE_ENDIAN_ENCODE_32(val));
    }

    SessionCloseEvent &closeReason(const CloseReason::Value value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const SessionCloseEvent &_writer)
{
    SessionCloseEvent writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "SessionCloseEvent", )";
    ...
    builder << ", ";
    builder << R"("closeReason": )";
    builder << '"' << writer.closeReason() << '"';
