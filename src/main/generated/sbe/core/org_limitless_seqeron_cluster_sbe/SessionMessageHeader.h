...
#ifndef _ORG_LIMITLESS_SEQERON_CLUSTER_SBE_SESSIONMESSAGEHEADER_CXX_H_
#define _ORG_LIMITLESS_SEQERON_CLUSTER_SBE_SESSIONMESSAGEHEADER_CXX_H_
...
public:
    ...
    SessionMessageHeader(char *buffer, const std::uint64_t bufferLength) :
        SessionMessageHeader(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    SessionMessageHeader(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        SessionMessageHeader(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    SessionMessageHeader &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    SessionMessageHeader &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    SessionMessageHeader &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    SessionMessageHeader &sbeRewind()
    {
    ...
    {
        SessionMessageHeader skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    SessionMessageHeader &leadershipTermId(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    SessionMessageHeader &clusterSessionId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    SessionMessageHeader &timestamp(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const SessionMessageHeader &_writer)
{
    SessionMessageHeader writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "SessionMessageHeader", )";
