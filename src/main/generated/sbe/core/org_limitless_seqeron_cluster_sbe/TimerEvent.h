...
#ifndef _ORG_LIMITLESS_SEQERON_CLUSTER_SBE_TIMEREVENT_CXX_H_
#define _ORG_LIMITLESS_SEQERON_CLUSTER_SBE_TIMEREVENT_CXX_H_
...
public:
    ...
    TimerEvent(char *buffer, const std::uint64_t bufferLength) :
        TimerEvent(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    TimerEvent(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        TimerEvent(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    TimerEvent &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    TimerEvent &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        TimerEvent skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    TimerEvent &leadershipTermId(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    TimerEvent &correlationId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    TimerEvent &timestamp(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const TimerEvent &_writer)
{
    TimerEvent writer(
        _writer.m_buffer,
        _writer.m_offset,
