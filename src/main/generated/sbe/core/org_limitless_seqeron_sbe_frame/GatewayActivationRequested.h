...
#ifndef _ORG_LIMITLESS_SEQERON_SBE_FRAME_GATEWAYACTIVATIONREQUESTED_CXX_H_
#define _ORG_LIMITLESS_SEQERON_SBE_FRAME_GATEWAYACTIVATIONREQUESTED_CXX_H_
...
class GatewayActivationRequested
{
private:
...
public:
    ...
    GatewayActivationRequested() = default;

    GatewayActivationRequested(
        char *buffer,
    ...
    GatewayActivationRequested(char *buffer, const std::uint64_t bufferLength) :
        GatewayActivationRequested(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    GatewayActivationRequested(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        GatewayActivationRequested(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    GatewayActivationRequested &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    GatewayActivationRequested &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    GatewayActivationRequested &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    GatewayActivationRequested &sbeRewind()
    {
    ...
    {
        GatewayActivationRequested skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    GatewayActivationRequested &gatewayId(const std::int32_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const GatewayActivationRequested &_writer)
{
    GatewayActivationRequested writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "GatewayActivationRequested", )";
