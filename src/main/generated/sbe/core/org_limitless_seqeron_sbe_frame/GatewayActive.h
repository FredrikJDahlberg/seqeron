...
#ifndef _ORG_LIMITLESS_SEQERON_SBE_FRAME_GATEWAYACTIVE_CXX_H_
#define _ORG_LIMITLESS_SEQERON_SBE_FRAME_GATEWAYACTIVE_CXX_H_
...
public:
    ...
    GatewayActive(char *buffer, const std::uint64_t bufferLength) :
        GatewayActive(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    GatewayActive(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        GatewayActive(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    GatewayActive &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    GatewayActive &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        GatewayActive skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
public:
    ...
    GatewayActive &gatewayId(const std::int32_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const GatewayActive &_writer)
{
    GatewayActive writer(
        _writer.m_buffer,
        _writer.m_offset,
