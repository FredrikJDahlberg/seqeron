...
#ifndef _ORG_LIMITLESS_SEQERON_SBE_FRAME_SEQUENCEDSYSTEMHEADER_CXX_H_
#define _ORG_LIMITLESS_SEQERON_SBE_FRAME_SEQUENCEDSYSTEMHEADER_CXX_H_
...
public:
    ...
    SequencedSystemHeader() = default;

    SequencedSystemHeader(
        char *buffer,
    ...
    {
        if (SBE_BOUNDS_CHECK_EXPECT(((m_offset + 34) > m_bufferLength), false))
        {
    ...
    SequencedSystemHeader(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        SequencedSystemHeader(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    SequencedSystemHeader(
        char *buffer,
        const std::uint64_t bufferLength) :
        SequencedSystemHeader(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }

    SequencedSystemHeader &wrap(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    ...
    SequencedSystemHeader &sourceId(const std::int32_t value) SBE_NOEXCEPT
    {
    ...
    SequencedSystemHeader &connectionId(const std::int32_t value) SBE_NOEXCEPT
    {
        std::int32_t val = SBE_LITTLE_ENDIAN_ENCODE_32(value);
    ...
    SequencedSystemHeader &sessionId(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    SequencedSystemHeader &systemEventType(const std::uint16_t value) SBE_NOEXCEPT
    {
    ...
    SBE_NODISCARD static const char *globalSeqNoMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static SBE_CONSTEXPR std::uint16_t globalSeqNoId() SBE_NOEXCEPT
    {
        return -1;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t globalSeqNoSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool globalSeqNoInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t globalSeqNoEncodingOffset() SBE_NOEXCEPT
    {
        return 18;
    }

    static SBE_CONSTEXPR std::int64_t globalSeqNoNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_INT64;
    }

    static SBE_CONSTEXPR std::int64_t globalSeqNoMinValue() SBE_NOEXCEPT
    {
        return INT64_C(-9223372036854775807);
    }

    static SBE_CONSTEXPR std::int64_t globalSeqNoMaxValue() SBE_NOEXCEPT
    {
        return INT64_C(9223372036854775807);
    }

    static SBE_CONSTEXPR std::size_t globalSeqNoEncodingLength() SBE_NOEXCEPT
    {
        return 8;
    }

    SBE_NODISCARD std::int64_t globalSeqNo() const SBE_NOEXCEPT
    {
        std::int64_t val;
    ...
    SequencedSystemHeader &globalSeqNo(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    SequencedSystemHeader &timestamp(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, SequencedSystemHeader &writer)
{
    builder << '{';
    builder << R"("sourceId": )";
    ...
    builder << ", ";
    builder << R"("globalSeqNo": )";
    builder << +writer.globalSeqNo();
