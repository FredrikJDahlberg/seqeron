...
// core, but every seqeron C++ process (FixGateway, OrderExecServer, BasicDataServer) shares a
...
// wraps the choice in a std::variant so it becomes a SEQERON_IDLE_STRATEGY env var instead, matching
...
// Resolves SEQERON_IDLE_STRATEGY (case-insensitive): "backoff" (default), "yielding", or "busyspin".
inline DynamicIdleStrategy resolveIdleStrategy()
{
    const char* v = std::getenv("SEQERON_IDLE_STRATEGY");
    ...
    throw std::invalid_argument("Unknown SEQERON_IDLE_STRATEGY=" + name +
