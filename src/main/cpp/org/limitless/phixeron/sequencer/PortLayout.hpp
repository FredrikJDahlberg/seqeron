...
inline constexpr std::uint16_t FIX_TEST_CLIENT_EGRESS_PORT = 9320;   // fix_test_server's own (non-colocated) cluster egress
...
constexpr std::uint16_t fixTcpPort(int gatewayIndex = 0)
{
    return static_cast<std::uint16_t>(FIX_TCP_PORT_BASE + gatewayIndex);
}
constexpr std::uint16_t orderExecEgressPort(int memberId)
{
    return static_cast<std::uint16_t>(ORDER_EXEC_EGRESS_PORT_BASE + memberId);
}
constexpr std::uint16_t fixGatewayEgressPort(int memberId)
{
    return static_cast<std::uint16_t>(FIX_GATEWAY_EGRESS_PORT_BASE + memberId);
}
constexpr std::uint16_t basicDataEgressPort(int memberId)
{
